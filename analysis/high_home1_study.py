...
for season in SEASONS:
  for league in LEAGUES:
    ...
    for row in download(season,league):
        ...
        hitem={'pts':hp,'win':int(hg>ag),'loss':int(hg<ag)}; aitem={'pts':ap,'win':int(ag>hg),'loss':int(ag<hg)}
...
mask=[]
for _,fn in conditions: mask.append({i for i,r in enumerate(records) if fn(r)})

def intersect(idxs,base):
    s=base & mask[idxs[0]]
    for j in idxs[1:]:
        if not s: break
        s &= mask[j]
...
results=[]; odds_idx=range(9); football_idx=range(9,len(conditions))
for oi in odds_idx:
  for k in range(1,5):
    for comb in itertools.combinations(football_idx,k):
      idxs=(oi,)+comb
      tr=intersect(idxs,train_all); trn=len(tr)
      if trn<100: continue
      tw=len(tr&wins); trrate=tw/trn
      if trrate<0.66: continue
      te=intersect(idxs,test_all); ten=len(te)
      if ten<35: continue
      ew=len(te&wins); terate=ew/ten
      if terate<0.62: continue
      cn=trn+ten; cw=tw+ew; cr=cw/cn
      score=wilson(ew,ten)*0.65+wilson(tw,trn)*0.35
      results.append((score,cr,trrate,terate,trn,ten,cn,cw,idxs))

results.sort(reverse=True,key=lambda x:(x[0],x[1],x[6]))
print('QUALIFYING_RULES',len(results)); print('\nTOP_RULES')
for rank,res in enumerate(results[:30],1):
    score,cr,trr,ter,trn,ten,cn,cw,idxs=res; names=[conditions[i][0] for i in idxs]
    allset=intersect(idxs,set(range(len(records)))); avgodd=statistics.mean(records[i]['odd'] for i in allset)
...
base={i for i,r in enumerate(records) if 1.50<=r['odd']<=2.50}; bw=len(base&wins)
print('\nBASE_1_50_2_50',len(base),f'{100*bw/len(base):.1f}%')
for j,(name,fn) in enumerate(conditions[9:],9):
    s=base & mask[j]
    if len(s)>=100: print(f'SINGLE | {name} | n={len(s)} | 1={100*len(s&wins)/len(s):.1f}%')
